...
error_reporting(0);
 ...
 if (isset($_POST['submit'])) {
    ...
    $appno=$_POST['appno'];
     ...
     $comments ='Your transcript request has been treated and forwarded to file office for further process';


     $sql3="INSERT INTO trans_docs(invoiceno,appno, sessionadmin,sessiongrad,comments,name,section,update_time,status)VALUES('$invoiceno','$appno', '$sessionadmin', '$sessiongrad', '$comments', '$officer', '$tasks', now(), 'treated')";
     
     $conn->query($sql3);
     $last_id = mysqli_insert_id($conn);
     if ($last_id) {
         $sql2="UPDATE trans_docs SET status='ok' WHERE invoiceno='$invoiceno' AND status='comment'";
         $sql="UPDATE transinvoice SET invoice_flag ='1' WHERE invoiceno='$invoiceno' AND invoice_flag='0'";
         ...
         sendMail($email, $surname, $othername, $comments, $officer, $update_time);
        ...
        <div class="content-wrapper">
            ...
            <div class="container-fluid">
                ...
                <?php if (!empty($start) && !empty($end)) {
                    ...
                    <?php }else{?>
                        <a href="request.php" class="btn btn-primary"><i class="fas fa-arrow-circle-left"></i> back</a>
                    <?php }?>
                </div><!-- /.col -->
                <div class="col-sm-6">
