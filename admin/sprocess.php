...
error_reporting(0);
 ...
 if (isset($_POST['submit'])) {
    ...
    $appno=$_POST['appno'];
     ...
     $sql3="INSERT INTO trans_docs(invoiceno,appno, sessionadmin,sessiongrad,comments,name,section,update_time,status)VALUES('$invoiceno','$appno', '$sessionadmin', '$sessiongrad', 'Your transcript request has been signed and forwarded to dispatch office for further process', '$officer', '$tasks', now(), 'treated')";
     
     $conn->query($sql3);
     $last_id = mysqli_insert_id($conn);
     if ($last_id) {
         $sql2="UPDATE trans_docs SET status='ok' WHERE invoiceno='$invoiceno' AND status='comment'";
         $sql="UPDATE transinvoice SET invoice_flag ='5' WHERE invoiceno='$invoiceno' AND invoice_flag='4'";
