 ...
 if (isset($_POST['update'])) {
  ...
  $sql="SELECT * FROM biodata WHERE id='".$_POST["form_no"]."'";
    ...
    if ($row) {
     ...
     $room=$_POST['room'];
     $id=$_POST['id'];
  $sql="UPDATE biodata SET status='$status', room='', room_category='', comments='$comments', updated_time=now() WHERE id='$form_no'";
  $sql1="UPDATE rooms SET status='$status' WHERE id='$id'";
  $conn->query($sql);
  $conn->query($sql1);
...
<div style="max-width: 500px; margin: auto;">
   ...
   <form method="POST" action="">
    ...
    <label>SERIAL NO.</label>
      ...
      <input type="hidden" value="<?php echo $row['room']; ?>" name="id" class="" placeholder="" readonly="">

   <br>
   <label>EJECT STUDENT</label>
   ...
   <textarea name="comments" required="" style="width: 100%; resize: none;"></textarea>
     <button type="submit" class="btn btn-danger btn-block" name="update" style="margin-bottom: 20px;">EJECT STUDENT</button>
    </form>
    <?php }}?> 
    </div>
      </div>
